...
public class Login extends javax.swing.JFrame {
    ...
    private void btSignInActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSignInActionPerformed
       ...
       else if(getStateStaff(user).equals("1"))
       {
       ...
       else if( checkUser(user)==true && checkPass(user, pass)==true)
       {
          ...
          // Image img=new ImageIcon()
           ...
           a.setUserName(user);
           GetStaffIDFromUser(user);
           System.out.println(getIdStaff());
           a.setIdStaff(getIdStaff());
...
public void GetStaffIDFromUser(String user)
{
        try {
        Connection c=Connecting.Connect();
        String sql="Select MANV from NHANVIEN Where USERNAME='"+user+"'";
          PreparedStatement pr = c.prepareStatement(sql);
          ResultSet rs=pr.executeQuery();
            while(rs.next())
            {
                setIdStaff(rs.getString(1));
            }
    ...
    public String getStateStaff(String user)
   {
        try {

            Connection c=Connect();
            String sql="Select TRANGTHAI from NhanVien where USERNAME='"+user+"'";
            PreparedStatement pr=c.prepareStatement(sql);
            ResultSet rs=pr.executeQuery();
            while(rs.next())
            {
    ...
    private void txtPasswordKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtPasswordKeyPressed
       ...
       else if( checkUser(user)==true && checkPass(user, pass)==true)
       {
        ...
        else if (getPosition().equals("Quản kho"))
        {
            ...
            a.lbSell.setVisible(false);
             a.setLayout(null);
            a.lbInforGuestCar.setBounds(a.lbInforGuestCar.getX(),350,a.lbInforGuestCar.getWidth(),a.lbInforGuestCar.getHeight());
             a.lbStock.setBounds(a.lbStock.getX(),200,a.lbStock.getWidth(),a.lbStock.getHeight());
