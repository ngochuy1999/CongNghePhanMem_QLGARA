...
boolean check;
    ...
    initComponents();
            //Ban đầu cho các lb ẩn
             AutoCompleteDecorator.decorate(cbAccessoriName);
   ...
   this.getContentPane().setBackground(new Color(209, 209, 224));
    ...
    public void getComboBoxNameAccessori()
    {
        try {
            ...
            String sql=" select TENLK, TRANGTHAI from LinhKien";
        ...
        public void getComboBoxNameAccessoriWithUpdate()
    {
        try {
            Connection connect=Connect();
            String sql=" select TENLK, SOLUONG,TRANGTHAI from LinhKien";
            PreparedStatement pr=connect.prepareStatement(sql);
            ResultSet rs=pr.executeQuery();
            cbAccessoriName.removeAllItems();
            while(rs.next())
            {
                if(!rs.getString(2).equals("0") && rs.getString(3).equals("0"))
                {
